...
public class Producer implements Runnable {
    ...
    private static int produzirItem(){
    ...
    private void depositarItem(int item) {
        ...
        for (int i = 0; i < Buffer.bufferSize; i++) {
            ...
            if (Buffer.isOccupied[i] == false ){
                ...
                System.out.println("A thread " + Thread.currentThread().getName() + " depositou o item " + item + " na posição " + i + " do buffer");
    ...
    @Override
    public void run() {
        Thread.currentThread().setName("Produtora " + Thread.currentThread().getId());
